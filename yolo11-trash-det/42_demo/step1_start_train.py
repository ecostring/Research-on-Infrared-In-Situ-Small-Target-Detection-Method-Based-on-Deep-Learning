...
AAA  = r"D:\桌面\yolo11-trash-det\ultralytics\cfg\datasets\A_my_data.yaml"
model = YOLO("CG-YOLO11.yaml") # load a pretrained model (recommended for training)
results = model.train(data=AAA, project="./runs", epochs=100, imgsz=640, device=[], workers=0, batch=4, cache=True, amp=False)  # CPU 开始训练
...
model = YOLO("yolo8n.yaml") # load a pretrained model (recommended for training)
results = model.train(data=AAA, project="./runs", epochs=100, imgsz=640, device=[], workers=0, batch=4, cache=True, amp=False)  # CPU 开始训练
time.sleep(10) # 睡眠10s，主要是用于服务器多次训练的过程中使用
#
model = YOLO("yolo11.yaml") # load a pretrained model (recommended for training)
results = model.train(data=AAA, project="./runs", epochs=100, imgsz=640, device=[], workers=0, batch=4, cache=True, amp=False)  # CPU 开始训练
time.sleep(10) # 睡眠10s，主要是用于服务器多次训练的过程中使用
#
